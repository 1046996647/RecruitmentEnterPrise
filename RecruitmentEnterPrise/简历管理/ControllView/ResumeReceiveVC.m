...
- (void)get_sendresume
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@p/%ld",Get_sendresume,self.pageNO];
...
- (void)delAction
{
    ...
    for (ResumeModel *model in self.selectedArr) {
        [idArr addObject:model.sendresumeId];
...
-(void)buttonTouchedOnCell:(ZFTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath atButtonIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex == 0){
        ...
        [paraDic setValue:model.sendresumeId forKey:@"sendresumeId"];
